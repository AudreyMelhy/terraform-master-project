...
function disable_password_authentication {
    # Check if password authentication is already disabled
    if grep -q "PasswordAuthentication no" /etc/ssh/sshd_config; then
        echo "Password authentication is already disabled."
    else
        # Disable password authentication by modifying the SSH configuration file
        sudo sed -i 's/PasswordAuthentication yes/PasswordAuthentication no/' /etc/ssh/sshd_config
        echo "Password authentication has been disabled in /etc/ssh/sshd_config."
